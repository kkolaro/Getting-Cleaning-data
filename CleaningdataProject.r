# Source of data for this project: https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip
 ...
 YTT<-as.data.frame(YTT)

 YTT<-merge(x = YTT, y = activi, by = "V1", all.x =TRUE,sort = FALSE)
 ...
 #  each activity and each subject.
   ...
   uniqueindex<-unique(YTT)
   tidydata<-as.data.frame(tidydata)
   row=1
   for (a in 1:uniqactivities) {
     for(s in 1:nmbrsubjects){
       tidydata1[row,1]=uniqsubjects[s,1]
       tidydata1[row,2]=uniqueindex[a,1]
       tidydata1[row,3]=uniqueindex[a,2]
       
      temp <- tidydata[tidydata$Subject==uniqsubjects[s,1]&tidydata$Index==uniqueindex[a, 1], ]
